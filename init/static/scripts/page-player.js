...
function PagePlayer(oConfigOverride) {
  ...
  sm.debugMode = false;//(window.location.href.toString().match(/debug=1/i)?true:false); // enable with #debug=1 for example
  ...
  this.events = {
    ...
    onload: function() {
      if (!this.loaded) {
		...
		//soundtree changes
		oTemp.innerHTML = oString+' <span style="font-size:0.5em"> | this track seems to have moved, we\'re aware and are working on it :: '+(sm.sandbox.noRemote?' Possible cause: Flash sandbox is denying remote URL access.':(sm.sandbox.noLocal?'Flash denying local filesystem access':'404?'))+'</span>';
		audioError(oTemp.id, oTemp.href, 0);
		setTimeout(function(){
		  oTemp.innerHTML = oString;
		  pl.events.finish.apply(oThis); // load next
		},1000);
		//end of soundtree changes
	  } else {
  ...
  this.handleClick = function(e) 
  {
    ...
    // soundtree adjustments
    if (self.hasClass(o,'exclude')) {
      if (isIE && o.onclick) {
    ...
    {
	    //youtube media was clicked
        //first make sure that sound manager 2 isn't playing anything
        if(self.lastSound)
	...
	//use reg ex to validate that this is an audio id
	if(self.idRegEx.test(o.id))
		soundURL = getAudio(o.id);

	if(!soundURL || ! sm.canPlayURL(soundURL))
		return true;

    // end of sound tree adjustments
  ...
  //sound tree addition
  //allows you to add a link to the playlist dynamically
  this.add = function(id, oLink)
  {	if ((sm.canPlayURL(oLink.href) || self.hasClass(oLink,'playable')) && !self.hasClass(oLink,'exclude')) 	
	{
		//set the rel attribute with the id
	        oLink.rel = id;
		//add the link to the links array
	        self.links[id] = oLink;
		//apply the css class for decoration
		self.addClass(oLink,self.css.sDefault); // add default CSS decoration
	}
  }

  //sound tree addition
  //removes a entry from the playlist  
  this.remove = function(rel)
  {
	//remove the link entry for rel
	self.links[rel] = undefined;
	//remove the sound entry for rel
	self.soundsByObject[rel] = undefined;
  }

  //sound tree addition
  //recreates the playlist based on the links currently in playlist ul
  this.rebuildPlaylist = function()
  {
	...
	for (var i=0; i < oLinks.length; i++) 
		if ((sm.canPlayURL(oLinks[i].href) || self.hasClass(oLinks[i],'playable')) && !self.hasClass(oLinks[i],'exclude') && oLinks[i].rel != '')
		{

			sound = self.getSoundByObject(oLinks[i]);	
			if(sound && sound != -1)
				sound._data.nIndex = self.links.length;
  ...
  this.init = function() {
    ...
    for (var i=0; i<oLinks.length; i++) {
	//sound tree additions
	//add last conditional check to turn off the link searching and numbering(in rel field) functionality in the case of links that already have a rel assigned (dynamic cases)
      if ((sm.canPlayURL(oLinks[i].href) || self.hasClass(oLinks[i],'playable')) && !self.hasClass(oLinks[i],'exclude') && oLinks[i].rel == '') 	
      {
	//soundtree adjustment
	//thought this was cleaner since the rel counts are now complete sequences of numbers and don't skip any
        oLinks[i].rel = self.links.length;
	//end of change
        self.links[self.links.length] = oLinks[i];
...
//soundtree additions
//changed this check to support dynamic playlist additions
